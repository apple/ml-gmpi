    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        for tmp_start_idx in tqdm.tqdm(range(0, all_n_planes, chunk_n_planes)):
            ...
            mb_mpi_rgbas.append(tmp_mpi_rgbas.cpu())
...
def main(opt):
    ...
    if opt.dataset in ["FFHQ256"]:
        dummy_tex_size = 256
    elif opt.dataset in ["FFHQ512", "AFHQCat"]:
        dummy_tex_size = 512
    elif opt.dataset in ["FFHQ1024", "MetFaces"]:
        dummy_tex_size = 1024
    else:
        raise ValueError
    ...
    (
        ...
        config, metadata, opt.nplanes, dummy_tex_size, mpi_return_single_res_xyz, mpi_xyz_only_z, device
    )

    print("\n", metadata["h_stddev"], metadata["v_stddev"], "\n")

    mpi_renderer = MPIRenderer(
        n_mpi_planes=n_mpi_actual_planes,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--chunk_n_planes", type=int, default=64, help="reduce this number if the GPU memory is not large enough."
    )
