    ...
    def __init__(self,
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            # NOTE: we must change this name to avoid shape mismatch when loading weights
            self.torgb = ToRGBLayer(out_channels, 3, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last)
            self.todepth = ToRGBLayer(out_channels, 1, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last)
            self.num_torgb += 1
                ...
                if self.build_background_from_rgb:
                    with torch.no_grad():
                        if self.resolution >= 0:
                            ...
                            cur_background_x = torch.zeros(x.shape, device=x.device, requires_grad=False)
                            ...
                            if tmp_start_col < tmp_end_col:
                                ...
                                tmp_right_ratios = (tmp_cols - tmp_start_col) / (tmp_end_col - tmp_start_col)
                    ...
                    y_background = self.torgb(cur_background_x, w_rgba, fused_modconv=fused_modconv)
                else:
                    raise ValueError
            else:
                y_background = None

            y_rgb = self.torgb(x, w_rgba, fused_modconv=fused_modconv)
            y_alpha = self.todepth(x, w_rgba, fused_modconv=fused_modconv)
            if y_background is None:
                y = torch.cat((y_rgb, y_alpha), dim=1)
            else:
                y = torch.cat((y_rgb, y_background, y_alpha), dim=1)
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        # self.output_side_look_vol = output_side_look_vol

        # Single RGB + alpha
        self.torgba_sep_background = torgba_sep_background

        self.depth2alpha_z_range = depth2alpha_z_range
        self.depth2alpha_n_z_bins = depth2alpha_n_z_bins

        self.G_final_img_act = G_final_img_act
        assert self.G_final_img_act in ["none", "sigmoid", "tanh"], f"{self.G_final_img_act}"

        if self.torgba_sep_background:
            # img_channels = n_planes * 1 + 3 + 3
            img_channels = 1 + 3 + 3
        else:
            # img_channels = n_planes * 1 + 3
            img_channels = 1 + 3
    ...
    def forward(self, z, c, mpi_xyz_coords, xyz_coords_only_z, n_planes, truncation_psi=1, truncation_cutoff=None,
        ...
        z_diff_min = -1 * self.depth2alpha_z_range / self.depth2alpha_n_z_bins
        z_diff_max = self.depth2alpha_z_range / self.depth2alpha_n_z_bins
        ...
        if self.torgba_sep_background:
            ...
            depth = img[:, 6:7, ...]
            # [B, #planes - 1, 3, H, W]
            rgb = rgb.unsqueeze(1).expand(-1, self.n_planes - 1, -1, -1, -1)
            # [B, #planes, 3, H, W]
            rgb = torch.cat((rgb, background), dim=1)
        else:
            background = None
            # [B, 1, H, W]
            depth = img[:, 3:4, ...]
            # [B, #planes, 3, H, W]
            rgb = rgb.unsqueeze(1).expand(-1, self.n_planes, -1, -1, -1)
        
        # compute alpha from depth
        # xyz_coords: [#planes, tex_h, tex_w, 3] -> [#planes, tex_h, tex_w]
        tex_z = mpi_xyz_coords[self.img_resolution][..., 2]
        # [B, #planes, tex_h, tex_w]
        z_diff = tex_z.unsqueeze(0) - depth
        z_diff = z_diff.clamp(min=z_diff_min, max=z_diff_max)
        alpha = (z_diff - z_diff_min) / (z_diff_max - z_diff_min + 1e-8)
        assert torch.min(alpha) >= 0.0, f"{torch.min(alpha)}"
        assert torch.max(alpha) <= 1.0, f"{torch.max(alpha)}"

        # [B, #planes, 1, H, W]
        alpha = alpha.unsqueeze(2)

        img = torch.cat((rgb, alpha), dim=2)
